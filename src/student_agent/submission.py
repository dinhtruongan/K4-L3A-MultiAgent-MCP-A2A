...
def validate_artifacts(
    ...
    for number, line in enumerate(trace_lines, 1):
        ...
        if event["event_type"] == "tool_result_consumed":
            consumed[event["case_id"]].update(event.get("evidence_refs") or [])
        elif event["event_type"] == "case_finalized":
    ...
    for case_id, output in outputs.items():
        ...
        if not refs or not refs <= consumed[case_id]:
            raise ValueError(f"outputs/{case_id}.json has unconsumed or missing evidence refs")
        if case_id not in finalized:
            raise ValueError(f"traces/trace.jsonl has no case_finalized for {case_id}")
